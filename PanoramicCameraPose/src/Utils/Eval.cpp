...
namespace Utils
{
    ...
    std::tuple<double, double, double, double> ParseStrToRT(std::string& str, double corRot)
    {
        ...
        double degree = std::stod(matches[0]) + corRot;
    ...
    openMVG::geometry::Pose3 ParseStrToPose(std::string& str, double corRot)
    {
        // Step 1: Separate rotation angle and translation  
        auto[ degree, x, y, z ] = ParseStrToRT(str,corRot);
