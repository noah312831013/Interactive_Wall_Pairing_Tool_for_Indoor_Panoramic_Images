...
{
public:
	void SetPano01Info(const std::filesystem::path& filepath, int offset=0) 
	{ 
		m_pano01_path = filepath; 
		LoadTextureFromFile(m_pano01_path, &m_pano01_tex.texID, &m_pano01_tex.width, &m_pano01_tex.height,offset);
		DisableShowCorners();
	}
	void SetPano02Info(const std::filesystem::path& filepath,int offset=0)
	{ 
		m_pano02_path = filepath; 
		LoadTextureFromFile(m_pano02_path, &m_pano02_tex.texID, &m_pano02_tex.width, &m_pano02_tex.height,offset);
		DisableShowCorners();
	}
	...
	static bool LoadTextureFromFile(const std::filesystem::path& filepath, GLuint* out_texture, int* out_width, int* out_height, int offset);
