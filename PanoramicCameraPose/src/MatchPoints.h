...
struct MatchPoints
{
	...
	void show() 
	{
		std::cout << "Left Pixels:\n";
		for (const auto& pixel : left_pixels) {
			std::cout << "  (" << pixel.x << ", " << pixel.y << ")\n";
		}

		// Print right pixels
		std::cout << "Right Pixels:\n";
		for (const auto& pixel : right_pixels) {
			std::cout << "  (" << pixel.x << ", " << pixel.y << ")\n";
		}
		// Print weights
		std::cout << "Weights:\n";
